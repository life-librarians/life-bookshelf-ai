...
위의 정보를 바탕으로, 자서전을 더욱 풍부하게 만들 수 있는 다음 인터뷰 질문 하나를 생성해주세요.
...
chapter_prompt = PromptTemplate.from_template(
    """당신은 사용자의 자서전을 돕는 인터뷰 질문 생성 및 챕터 요약 AI입니다.
...
위의 정보를 바탕으로, 다음 질문과 함께 이번 대화 내용을 하나의 챕터로 요약하고, 챕터 제목까지 생성해주세요.

조건:
- 질문은 사용자의 이전 답변과 중복되지 않은 새로운 질문
- 감정, 동기, 배경, 맥락을 자연스럽게 회상하게 할 것
- 질문은 대화 흐름상 자연스러워야 함
- 챕터 제목은 간결하고 주제와 연결된 제목이어야 함
- 챕터 요약은 이 대화에서 다룬 핵심 내용과 감정을 포괄

응답은 아래 JSON 형식으로 출력:
```json
{{
  "question": "다음 인터뷰 질문",
  "chapter_title": "생성된 챕터 제목",
